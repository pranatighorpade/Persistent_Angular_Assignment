...
import { ListDataSucess, LIST_PRODUCTS, ListProducts, AddProduct, 
        AddProductSuccess,  DeleteProduct, DeleteProductSuccess, 
        UpdateProduct, DELETE_PRODUCT, UPDATE_PRODUCT, ADD_PRODUCT, UpdateProductSuccess } from '../actions/product.action';
      ...
      ListProducts: Observable<any> =  this.actions.pipe(
                ...
                        mergeMap(payload => {
                            return this.productService.createProduct(payload).pipe(
                                map((data) => {
                                    console.log('data from effects in add product....', data);
                                    if (data) {
                        ...
                                mergeMap(payload => {
                                    return this.productService.deleteProduct(payload).pipe(
                                        map((data) => {
                                            console.log('data from effects in delete product....', data);
                                            if (data) {
                         ...
                                 mergeMap(payload => {
                                     return this.productService.updateProduct(payload.index,payload.newProduct).pipe(
                                        map((data) => {
                                            console.log('data from effects in add product....', data);
                                            if (data) {
